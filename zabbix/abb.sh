...
# Reads locally available CSV files, delivered via SSH Push or SSH Pull.
#
# Zabbix invokes this directly (no env vars from the template), so if
# ABB_CSV_PATH isn't the default below, either symlink the default path to
# the real one, or set ABB_CSV_PATH in zabbix-server's own environment
# (systemctl edit zabbix-server) — see INSTALL.md.
#
